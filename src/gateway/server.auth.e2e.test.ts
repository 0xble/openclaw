...
describe("gateway server auth/connect", () => {
  ...
  test("single approval captures pending node and operator roles for the same device", async () => {
    ...
    const pendingForDevice = pending.pending.filter(
    ...
    const pendingAfterApprovalForDevice = afterApproval.pending.filter(
      (entry) => entry.deviceId === identity.deviceId,
    );
    expect(pendingAfterApprovalForDevice).toEqual([]);
  ...
  test("allows operator.read connect when device is paired with operator.admin", async () => {
    ...
    const list = await listDevicePairing();
    const pendingForDevice = list.pending.filter((entry) => entry.deviceId === identity.deviceId);
