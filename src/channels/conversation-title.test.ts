...
describe("isDefaultThreadPlaceholder", () => {
  it("matches built-in placeholders", () => {
    expect(isDefaultThreadPlaceholder("Untitled")).toBe(true);
    expect(isDefaultThreadPlaceholder("new conversation")).toBe(true);
    expect(isDefaultThreadPlaceholder("general")).toBe(true);
  });

  it("rejects normal thread names", () => {
    expect(isDefaultThreadPlaceholder("release planning")).toBe(false);
  });
});
