...
async function readLatestAssistantReplyWithRetry(params: {
  ...
  const deadline = Date.now() + Math.max(0, Math.min(params.maxWaitMs, 15_000));
  while (Date.now() < deadline) {
    const result = await readLatestAssistantReply({ sessionKey: params.sessionKey });
    if (result?.trim()) {
      return result;
    }
    await new Promise((resolve) => setTimeout(resolve, RETRY_INTERVAL_MS));
...
function resolveAnnounceOrigin(
  ...
  const hasExplicitTarget = Boolean(
    typeof normalizedRequester?.to === "string" && normalizedRequester?.to.trim(),
  );
  if (
    normalizedRequester?.channel &&
    !isDeliverableMessageChannel(normalizedRequester.channel) &&
    !hasExplicitTarget
  ) {
  ...
  try {
    ...
    const completionThreadId =
      completionDirectOrigin?.threadId != null && completionDirectOrigin.threadId !== ""
        ...
        : undefined;
    const hasCompletionDirectTarget = Boolean(completionChannel && completionTo);
    ...
    if (
      ...
      if (hasCompletionDirectTarget) {
        sendParams.channel = completionChannel;
        sendParams.to = completionTo;
        if (completionThreadId) {
          sendParams.threadId = completionThreadId;
        }
      }
      if (completionDirectOrigin?.accountId) {
        sendParams.accountId = completionDirectOrigin.accountId;
      }
      await callGateway({
        method: "send",
        params: sendParams,
...
export async function runSubagentAnnounceFlow(params: {
  ...
  try {
    ...
    let waitedForChildRunToSettle = false;
    ...
    if (!expectsCompletionMessage && childSessionId && isEmbeddedPiRunActive(childSessionId)) {
      ...
      waitedForChildRunToSettle = true;
    ...
    if (waitedForChildRunToSettle && !reply?.trim()) {
      const assistantReply = await readLatestAssistantReplyWithRetry({
        sessionKey: params.childSessionKey,
        maxWaitMs: params.timeoutMs,
      });
    ...
    let resolutionOrigin = targetRequesterOrigin;
    if (!requesterIsSubagent) {
      const { entry } = loadRequesterSessionEntry(targetRequesterSessionKey);
      resolutionOrigin = resolveAnnounceOrigin(entry, targetRequesterOrigin);
    }
    ...
    const delivery = await deliverSubagentAnnouncement({
      ...
      completionDirectOrigin: targetRequesterOrigin ?? resolutionOrigin,
      directOrigin: resolutionOrigin,
