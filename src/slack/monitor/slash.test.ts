...
function createArgMenusHarness() {
  ...
  const app = {
    ...
    optionsHandlers: options,
    options(
      this: { optionsHandlers?: Map<string, (args: unknown) => Promise<void>> },
      id: string,
      handler: (args: unknown) => Promise<void>,
    ) {
      if (!this.optionsHandlers) {
        throw new Error("missing options handler context");
      }
      this.optionsHandlers.set(id, handler);
    },
  };
...
describe("Slack native command argument menus", () => {
  ...
  it("registers external_select options handlers with bound app context", () => {
    expect(harness.options.has("openclaw_cmdarg")).toBe(true);
  });
...
describe("Slack native command argument menu registration", () => {
  it("does not fail channel startup when options handler registration throws", async () => {
    const commands = new Map<string, (args: unknown) => Promise<void>>();
    const app = {
    ...
    expect(runtimeError).toHaveBeenCalledWith(
      expect.stringContaining("options registration failed"),
    );
  });
});
