...
async function applyAuthChoiceForTest(
  params: Parameters<typeof import("./auth-choice.js").applyAuthChoice>[0],
) {
  const { applyAuthChoice } = await import("./auth-choice.js");
  return applyAuthChoice(params);
}

async function resolvePreferredProviderForAuthChoiceForTest(choice: AuthChoice) {
  const { resolvePreferredProviderForAuthChoice } = await import("./auth-choice.js");
  return resolvePreferredProviderForAuthChoice(choice);
}
...
describe("applyAuthChoice", () => {
  ...
  it("stores openai-codex OAuth with email profile id", async () => {
    ...
    const result = await applyAuthChoiceForTest({
      authChoice: "openai-codex",
      config: {},
      prompter,
      runtime,
...
describe("resolvePreferredProviderForAuthChoice", () => {
  it("maps github-copilot to the provider", async () => {
    await expect(resolvePreferredProviderForAuthChoiceForTest("github-copilot")).resolves.toBe(
      "github-copilot",
    );
  });

  it("maps qwen-portal to the provider", async () => {
    await expect(resolvePreferredProviderForAuthChoiceForTest("qwen-portal")).resolves.toBe(
      "qwen-portal",
    );
  });

  it("returns undefined for unknown choices", async () => {
    await expect(
      resolvePreferredProviderForAuthChoiceForTest("unknown" as AuthChoice),
