...
- [FORK] Channels/Slack+Telegram: rename thread titles from real message content only to avoid non-message naming artifacts (`fork@ff1850802`).
- [FORK] Fork runtime tailoring: customize local bot/runtime packaging and reply/cron write-through behavior for private operation (`fork@066b124c0`).
- [FORK] Repository profile: remove public/community-facing surfaces for private fork operation (`fork@c15a2ca7c`).
- [FORK] Fork stability: harden private-fork config-path handling and expand related test/harness coverage (`fork@94bfa8ce8`).
- [LOCAL] Slack typing/send hardening in progress: add Slack-specific typing-mode override plumbing and tests to support `channels.slack.typingMode=never` + `channels.slack.streaming=false` behavior in Slack while preserving non-Slack defaults (uncommitted).
...
- [LOCAL] Agents/Thinking defaults: enforce default precedence as session override -> per-agent `thinkingDefault` -> global `agents.defaults.thinkingDefault` -> model fallback, and align `chat.history` with the same per-agent-aware resolution path for consistent channel-bound defaults.
